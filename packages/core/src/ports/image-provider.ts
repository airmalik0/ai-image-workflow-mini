...
/**
 * Выбор провайдера под модель ноды.
 *
 * Модель выбирается на ноде, а провайдеров в процессе может быть поднято
 * несколько. Без этого порта исполнение шло бы в один провайдер на процесс,
 * и список моделей в интерфейсе обещал бы то, чего сделать нельзя: выбор
 * `Fake Image` на стенде с OpenAI падал бы с «модель не поддерживается».
 *
 * Порт отдан ядру, потому что решение «эту ноду исполняет вот этот провайдер»
 * принимается при исполнении ноды — там же, где по модели собирается запрос
 * под возможности движка.
 */
export interface ProviderSelector {
  /**
   * Провайдер, которому принадлежит модель. `null` — модель не выбрана;
   * неизвестная модель отдаётся активному провайдеру, а не подменяется молча:
   * его отказ «такой модели нет» честнее подставленной чужой картинки.
   */
  forModel(model: string | null): ImageProvider
}
