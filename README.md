...
```bash
git clone <репозиторий> && cd ai-image-workflow-mini
docker compose up
```
...
- **Публичный демо-стенд** — площадка была выбрана, но аккаунт хостера заблокирован по
  задолженности на стороне владельца; вместо ссылки — `docker compose up`, который поднимает
  ровно то же самое.
