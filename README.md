...
Чтобы посмотреть механику графа, не расходуя потолок, выберите на ноде модель **Fake Image**:
её исполняет офлайновый провайдер, а ветвление, таймлайн, отмена и повтор при этом настоящие.
...
`IMAGE_PROVIDER` задаёт провайдера по умолчанию (`auto` берёт первого доступного:
OpenAI → Gemini → fake), но **исполняет ноду тот провайдер, которому принадлежит выбранная
на ней модель**: `executeNode` вычисляет модель (поле ноды → `defaults` пресета → «по умолчанию»)
и по ней берёт исполнителя из реестра. Поэтому список моделей в интерфейсе не обещает лишнего,
а заглушку можно выбрать моделью на стенде с боевым ключом — прогнать ветвление, отмену и retry,
не тратя квоту. Модель, которой нет ни у кого, уходит провайдеру по умолчанию: его «такой модели
нет» честнее подставленной чужой картинки.
...
- **Добавлен статус run'а `cancelled`.** В ТЗ их четыре. Пятый нужен, потому что отменённый
  запуск — это не `failed`: ничего не сломалось, его остановил пользователь. Требуемые четыре
  значения на месте и означают ровно то же, что в задании.
- **CRUD `/api/workflows` есть на сервере, но фронт его не вызывает.** Сохранение графа между
  сессиями задание не требует, а незаконченная кнопка «Сохранить» была бы хуже её отсутствия.
  Эндпоинты остались: граф — это данные, и `POST /api/runs` принимает как сам граф, так и
  `workflowId` уже сохранённого.
...
- **Референс у пресета из сида — всегда фон или фактура, никогда предмет.** Разница проверена
  на живом `gpt-image-2` и на генерации, и на редактировании: фоновый референс модель применяет
  как сцену (кружка со снимка остаётся собой, бетон меняется на бесшовный белый), а референс
  с предметом композитит в кадр — с нарисованной сферой вместо отредактированной фотографии
  приезжала сфера. Сфера из набора убрана, у каждого из пяти пресетов свой фон.
- **Модель вендора в пресете сида не прибита.** Иначе пресет становится миной: на стенде
  с ключом другого провайдера нода падает с `VALIDATION_FAILED`, хотя пользователь модель
  не выбирал. Поле в контракте есть и работает у пользовательских пресетов.
